...
async function registerDeviceOnBackend(token: string, type: 'android' | 'ios' | 'web') {
    console.log('📝 [TEST LOG] Registering device on backend:', { token, type });
    try {
        const response = await api.post('/mobcash/devices/', {
            registration_id: token,
            type: type
        });
        console.log('✅ [TEST LOG] Device registered successfully on backend:', response.data);
    } catch (error: any) {
        console.error('❌ [TEST LOG] Error registering device on backend:', error.message || error);
    }
}
    ...
    try {
        ...
        if (Capacitor.getPlatform() === 'android') {
            await LocalNotifications.createChannel({
                id: 'slater_foreground',
                name: 'slater_foreground',
            ...
            if (Capacitor.isNativePlatform()) {
                try {
                    ...
                    if (Capacitor.getPlatform() === 'android') {
                        ...
                        notificationOptions.notifications[0].channelId = 'slater_foreground';
                    }
